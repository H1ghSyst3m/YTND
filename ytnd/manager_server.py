...
class ConnectionManager:
    ...
    def set_event_loop(self, loop: asyncio.AbstractEventLoop):
        """Set the main event loop for thread-safe coroutine execution."""
        self._main_loop = loop
    
    def broadcast_to_user_threadsafe(self, user_id: str, message: dict):
        """Thread-safe version of broadcast_to_user for use from worker threads."""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    uid = cookies.get(SESSION_UID_COOKIE)
    sig = cookies.get(SESSION_SIG_COOKIE)
...
async def api_process_queue(
    ...
    """Start processing the download queue for a user (runs in background)"""
    if current["role"] != "admin" and user_id != current["uid"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    if _active_downloads.get(user_id, False):
        raise HTTPException(status_code=409, detail="Download already in progress for this user")
    ...
    async def run_download():
        ...
        try:
            ...
            logger.error(f"Download process failed for user {user_id}: {e}")
            await manager.broadcast_to_user(user_id, {
                "type": "download_error",
                "userId": user_id,
                "error": str(e)
...
async def startup_event():
    """Initialize the ConnectionManager with the main event loop on startup."""
